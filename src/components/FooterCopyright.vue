<script setup>
import AnimatedLinkwoArrow from './AnimatedLinkwoArrow.vue';
</script>
<template>
    <div class="h-[5] container flex flex-col lg:flex-row justify-between border-t">
        <div class="flex items-center w-full lg:w-2/5 justify-between">
            <AnimatedLinkwoArrow text="LinkedIn" href="https://www.linkedin.com/in/sebastian-pohl-09b42598/"></AnimatedLinkwoArrow>
            <AnimatedLinkwoArrow text="Behance" href="https://www.behance.net/spdesignfrontend"></AnimatedLinkwoArrow>
            <AnimatedLinkwoArrow text="GitHub" href="https://github.com/godchilla85-alt"></AnimatedLinkwoArrow>
            <AnimatedLinkwoArrow text="Instagram" href="https://www.instagram.com/sp.digital.arts/"></AnimatedLinkwoArrow>
        </div>
        <div class="flex items-center">
            <p class="mt-5">&copy; 2026 - Sebastian Pohl</p>
        </div>
    </div>
</template>